...
> Last updated: 2026-05-21 (afternoon) — **Realizacje PPF gallery shipped** — real workshop photos from Tomasz replace the 2-card "Wkrótce" placeholder on `/ppf`. 5 WhatsApp JPEGs (1.1 MB) optimized to WebP (516 KB total, ~53% off) and routed through `public/ppf/realizacje/`; raw originals moved to `assets-src/ppf/realizacje-originals/` (excluded from Vercel via `.vercelignore`). New `Realizacje.jsx` component: featured landscape hero card (1400×1050, hex-LED hood application) + 3-up portrait grid (rear squeegee / front film overlay / bonnet edge cut) for the cars, then a separate **Łodzie i sprzęt** sub-row anchoring the boat-polish photo with 3-bullet copy block (kadłub / pokład / wycena). Hover scale, gradient caption strip, lazy-load on everything except the hero. Build: 360.64 kB / 105.70 kB gzip JS · 64.25 kB / 11.58 kB gzip CSS (+3 kB JS / +2 kB CSS over prior). Prior `2026-05-21 (early hours)` entry preserved below.
>
